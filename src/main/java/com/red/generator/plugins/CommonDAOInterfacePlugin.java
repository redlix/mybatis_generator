 ...
 *
 * @author redli
 * @version 1.0.0 2019-05-16 08:24
 * @since 1.0.0
 */
@Slf4j
public class CommonDAOInterfacePlugin extends PluginAdapter {
    ...
    private static final FullyQualifiedJavaType PARAM_ANNOTATION_TYPE = new FullyQualifiedJavaType(
...
//    @Override
    public List<GeneratedJavaFile> contextGenerateAdditionalJavFiles(IntrospectedTable introspectedTable) {
        ...
        List<GeneratedJavaFile> mapperJavFiles = new ArrayList<>();
        ...
        if (stringHasValue(daoTargetPackage)) {
            ...
            FullyQualifiedJavaType daoBaseInterfaceJavType = mapperInterface.getType();
            daoBaseInterfaceJavType.addTypeArgument(new FullyQualifiedJavaType("Model"));
            daoBaseInterfaceJavType.addTypeArgument(new FullyQualifiedJavaType("PK extends Serializable"));

            if (isUseExample()) {
                daoBaseInterfaceJavType.addTypeArgument(new FullyQualifiedJavaType("E"));
            }
            ...
            GeneratedJavaFile mapperJavaFile = new GeneratedJavaFile(mapperInterface, daoTargetDir, javaFileEncoding,
                    javaFormatter);
            ...
            try {
                ...
                if (!mapperFile.exists()) {
                    mapperJavFiles.add(mapperJavaFile);
                }
            } catch (ShellException e) {
                log.error("exception: {}", e.getMessage());
            }
        }
        return mapperJavFiles;
    }
